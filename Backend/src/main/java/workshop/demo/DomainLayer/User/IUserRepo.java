...
public interface IUserRepo extends JpaRepository<Guest, Integer> {

    @Query("SELECT CASE WHEN COUNT(r) > 0 THEN true ELSE false END FROM Registered r WHERE r.username = :username")
    boolean existsRegisteredUserByUsername(@Param("username") String username);
    ...
    // Check if a Registered user exists with the given username (guestName)
    // @Query("SELECT COUNT(g) > 0 FROM Guest g WHERE TYPE(g) = Registered AND
    // g.guestName = :username")
    // boolean existsRegisteredUserByUsername(@Param("username") String username);

    // void saveRegistered(Registered user);
    ...
    // //===========
    // public void addItemToGeustCart(int guestId, ItemCartDTO item) throws
    // UIException;

    // public void removeItemFromGeustCart(int guestId, int productId) throws
    // UIException;

    // public void ModifyCartAddQToBuy(int guestId, int productId, int quantity)
    // throws UIException;
    ...
    // public void addSpecialItemToCart(UserSpecialItemCart item, int userId) throws
    // DevException, UIException;

    // public List<UserSpecialItemCart> getAllSpecialItems(int userId) throws
    // UIException;
    ...
    // public void checkUserRegisterOnline_ThrowException(int userId) throws
    // UIException;
    ...
    // public void removeSpecialItem(int userId, UserSpecialItemCart itemToRemove)
    // throws UIException;
