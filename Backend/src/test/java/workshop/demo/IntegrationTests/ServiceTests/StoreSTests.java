...
public class StoreSTests {
    ...
    // when(real.mockStockRepo.addProduct("Tablet", Category.ELECTRONICS, "10-inch
    // Tablet", keywords))
    // .thenReturn(productId);
    // int returnedProductId = real.stockService.addProduct(ownerToken, "Tablet",
    // Category.ELECTRONICS, "10-inch Tablet", keywords);
    // assertEquals(productId, returnedProductId);

    // // === Add item to store ===
    // when(real.mockIOSrepo.manipulateItem(ownerId, storeId,
    // Permission.AddToStock)).thenReturn(true);
    // when(real.mockStockRepo.findByIdInSystem_throwException(productId)).thenReturn(
    // new Product("Tablet", productId, Category.ELECTRONICS, "10-inch Tablet",
    // keywords));
    // when(real.mockStockRepo.addItem(storeId, productId, 10, 100,
    // Category.ELECTRONICS))
    // .thenReturn(new item(productId, 10, 100, Category.ELECTRONICS));

    // int itemAdded = real.stockService.addItem(storeId, ownerToken, productId, 10,
    // 100, Category.ELECTRONICS);
    // assertEquals(productId, itemAdded);
    // }

    // //todo:this case is not checked
    // @Test
    // void testOwner_AddProductToStock_Failure_InvalidProductData() throws
    // Exception {
    // int storeId = 100;
    // int productId = 200;
    ...
    // when(real.mockStockRepo.findByIdInSystem_throwException(productId)).thenReturn(
    // new Product("Phone", productId, Category.ELECTRONICS, "Smartphone", new
    // String[]{"Phone"})
    // );

    // // === Act & Assert ===
    // UIException ex = assertThrows(UIException.class, () ->
    // real.stockService.addItem(storeId, ownerToken, productId, -5, -999,
    // Category.ELECTRONICS)
    // );
    ...
    // .thenThrow(new UIException(" store does not exist.",
    // ErrorCodes.STORE_NOT_FOUND));

    // UIException ex = assertThrows(UIException.class, () ->
    // real.stockService.addItem(storeId, ownerToken, productId, 10, 100,
    // Category.ELECTRONICS)
    // );
    ...
    // when(real.mockIOSrepo.manipulateItem(ownerId, storeId,
    // Permission.DeleteFromStock)).thenReturn(true);
    ...
    // // Act
    // int removedProductId = real.stockService.removeItem(storeId, ownerToken,
    // productId);
    ...
    // when(real.mockIOSrepo.manipulateItem(ownerId, storeId,
    // Permission.DeleteFromStock)).thenReturn(false);
    ...
    // when(real.mockUserRepo.isRegistered(ownerId)).thenReturn(true);
    // when(real.mockIOSrepo.manipulateItem(ownerId, storeId,
    // Permission.UpdatePrice)).thenReturn(true);

    // when(real.mockStockRepo.updatePrice(storeId, productId,
    // 10)).thenReturn(true);
    ...
    // when(real.mockUserRepo.isRegistered(ownerId)).thenReturn(true);
    // when(real.mockIOSrepo.manipulateItem(ownerId, storeId,
    // Permission.UpdateQuantity)).thenReturn(true);
    // when(real.mockStockRepo.updateQuantity(storeId, productId,
    // 10)).thenReturn(true);
    ...
    // when(real.mockStockRepo.updateQuantity(storeId, productId,
    // -10)).thenThrow(new UIException("This worker is not authorized!",
    // ErrorCodes.NO_PERMISSION));
    // //real.stockService.updateQuantity(storeId, ownerToken, productId, -10),new
    // UIException("This worker is not authorized!", ErrorCodes.NO_PERMISSION));
    ...
    // // === Ownership logic ===
    // when(real.mockIOSrepo.checkToAddOwner(storeId, ownerId,
    // userId)).thenReturn(true);
    // when(real.mockIOSrepo.AddOwnershipToStore(storeId, ownerId,
    // userId)).thenReturn(true);

    // // === Approval messaging ===
    // //when(real.storeService.sendMessageToTakeApproval(ownerId,
    // userId)).thenReturn(true);

    // // === Act ===
    // int result = real.storeService.AddOwnershipToStore(storeId, ownerToken,
    // userId);
    ...
    // int first = real.storeService.AddOwnershipToStore(storeId, ownerToken,
    // userId);
    ...
    // doThrow(new UIException("You are not regestered user!",
    ...
    // assertDoesNotThrow(() -> real.storeService.DeleteOwnershipFromStore(storeId,
    // ownerToken, userId));
    ...
    // when(real.mockUserRepo.checkUserRegister_ThrowException(21)).thenThrow(new
    ...
    // doNothing().when(real.mockIOSrepo).checkToAddManager(storeId, ownerId,
    // managerId);
    // //doNothing().when(real.mockIOSrepo).AddManagerToStore(storeId, ownerId,
    // managerId);
    // List<Permission> a = new LinkedList<>();
    // a.add(Permission.AddToStock);
    // a.add(Permission.DeleteFromStock);
    // int result = real.storeService.AddManagerToStore(storeId, ownerToken,
    // managerId, a);
    ...
    // real.storeService.AddManagerToStore(storeId, ownerToken, nonExistentUserId,
    // a);
    // });
    // }
    ...
    // doThrow(new UIException("This worker is already an owner/manager",
    // ErrorCodes.NO_PERMISSION))
    ...
    // UIException ex = assertThrows(UIException.class, () -> {
    // real.storeService.AddManagerToStore(storeId, ownerToken, managerId,
    // permissions);
    // });
    ...
    // when(real.mockSusRepo.checkUserSuspensoin_ThrowExceptionIfSuspeneded(ownerId)).thenReturn(true);
    // when(real.mockUserRepo.checkUserRegisterOnline_ThrowException(managerId)).thenReturn(true);
    // when(real.mockStoreRepo.checkStoreExistance(storeId)).thenThrow(new
    // UIException(" store does not exist.", ErrorCodes.STORE_NOT_FOUND));
    // when(real.mockStoreRepo.checkStoreIsActive(storeId)).thenReturn(true);
    // //real.storeService.deleteManager(storeId,ownerToken,managerId);
    ...
    // when(real.mockUserRepo.checkUserRegisterOnline_ThrowException(managerId)).thenReturn(true);
    // when(real.mockStoreRepo.checkStoreExistance(storeId)).thenReturn(true);
    // when(real.mockStoreRepo.checkStoreIsActive(storeId)).thenThrow(new
    // DevException(" store is not active"));
    // //real.storeService.deleteManager(storeId,ownerToken,managerId);
    ...
    // when(real.mockStoreRepo.checkStoreExistance(storeId)).thenReturn(true);
    // when(real.mockStoreRepo.checkStoreIsActive(storeId)).thenThrow(new
    // DevException("Store not found with ID: " + storeId));
    // //real.storeService.deleteManager(storeId,ownerToken,managerId);
    ...
    // doNothing().when(real.mockIOSrepo).changePermissions(ownerId, managerId,
    // storeId, updatedPermissions);

    // assertDoesNotThrow(() -> {
    // real.storeService.changePermissions(ownerToken, managerId, storeId,
    // updatedPermissions);
    // });
    // }
    // @Test
    ...
    // UIException ex = assertThrows(UIException.class, () -> {
    // real.storeService.changePermissions(ownerToken, notManagerId, storeId,
    // permissions);
    // });
    ...
    // doThrow(new UIException("can't deactivate an DEactivated store",
    // ErrorCodes.DEACTIVATED_STORE))
    ...
    // @Test
    // void testOwner_ViewStorePurchaseHistory_Failure_StoreNotExist() throws
    // Exception {
    // int ownerId = 10;
    // int storeId = 100;
    // String ownerToken = "owner-token";
    ...
    // //todo:this case is not checked
    // @Test
    // void testOwner_DeleteProductFromStock_Failure_ProductNotFound() throws
    // Exception {
    // int storeId = 100;
    ...
    // .thenThrow(new UIException("Product not found!",
    ...
    // real.stockService.setProductToBid(ownerToken,storeId,productId,1)
    // );
    // assertEquals(ex.getMessage(),"you have no permession to set product to
    // bid.");
